...
public class GoReplayPackageTest {
    ...
    GoReplayPackage goReplayPackage;


    @Test
    public void responseWithTransaction() {
        goReplayPackage = new GoReplayPackage(RES_WITH_TRANSACTION);
        assertEquals("Package ID does not match", goReplayPackage.getId(), RES_ID);
        assertEquals("Transaction ID mismatch", goReplayPackage.getCompoundTransaction(), RES_TRANSACTION_ID);
        assertEquals("Payload modified", goReplayPackage.getPayload(), RES_WITH_TRANSACTION);
        goReplayPackage.replaceCompoundTransaction(RES_NEW_TRANSACTION_ID);
        goReplayPackage = new GoReplayPackage(goReplayPackage.getPayload());
        assertEquals("New transaction ID mismatch", goReplayPackage.getCompoundTransaction(), RES_NEW_TRANSACTION_ID);
    }

    @Test
    public void requestWithoutTransaction() {
        goReplayPackage = new GoReplayPackage(REQ_NO_TRANSACTION);
        assertEquals("Package ID does not match", goReplayPackage.getId(), REQ_ID);
        assertNull("Bogus transaction ID", goReplayPackage.getCompoundTransaction());
        assertFalse("Package is not part of a compound transaction", goReplayPackage.isCompoundTransaction());
        assertEquals("Incorrect payload type, should be 1", goReplayPackage.getType(),'1');
        goReplayPackage.replaceCompoundTransaction(RES_NEW_TRANSACTION_ID);
        assertEquals("Payload should be unchanged", goReplayPackage.getPayload(), REQ_NO_TRANSACTION);
    }
}
